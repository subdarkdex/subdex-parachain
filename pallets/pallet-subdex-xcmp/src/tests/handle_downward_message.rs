...
fn handle_downward_message() {
    with_test_externalities(|| {
        ...
        let transferred_tokens_from_relay_chain_event =
            get_test_event(RawEvent::TransferredTokensFromRelayChain(FirstAccountId::get(), transfer_amount));

        // Last event checked

        // deposit_creating() method of Currency trait emits 2 additional events
        assert_event_success(transferred_tokens_from_relay_chain_event, number_of_events_before_call + 3);
    })
}