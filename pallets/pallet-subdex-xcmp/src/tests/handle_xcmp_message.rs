...
fn handle_xcmp_message() {
    with_test_externalities(|| {
        ...
        let next_asset_id = next_asset_id();
        ...
        emulate_xcmp_message(FirstAccountId::get(), FirstAccountId::get(), transfer_amount, para_asset_id);
        ...
        let transferred_tokens_from_relay_chain_event =
            get_test_event(
                RawEvent::DepositAssetViaXCMP(FirstParaId::get(), para_asset_id, FirstAccountId::get(), next_asset_id, transfer_amount));

        // Last event checked (number_of_events_before_call is incremented by two, because we received 2 xcmp messages)
        assert_event_success(transferred_tokens_from_relay_chain_event, number_of_events_before_call + 2);
    })
}